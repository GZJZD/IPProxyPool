...
def detect_proxy(selfip, proxy, queue2=None):
    ...
    if config.CUSTOM_ENABLE:
        # 调用自定义的检查函数
        c_protocol, c_speed = getattr(sys.modules[__name__], config.CUSTOM_CHECK_PROXY['function'])(selfip, proxies)

        if protocol > 0 and c_protocol > 0:
            proxy['protocol'] = protocol
            proxy['types'] = types
            proxy['speed'] = c_speed
        else:
...
def xueqiu_check(selfip, proxies):
    try:
        start = time.time()
        r = requests.get(url=config.CUSTOM_TEST_HTTPS, headers=config.get_header(), timeout=config.TIMEOUT, proxies=proxies)
